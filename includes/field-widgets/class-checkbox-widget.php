...
class WPF_Checkbox_Widget extends WPF_Base_Widget {  
  ...
  public function __construct( $field, $product_id, $default_value = '', $is_admin_form = false ) {
    ...
    if ( $is_admin_form ) {
      ...
      $this->price = $field['charge'];
      $this->default_value = $field['default_value'];
      $this->default_price = $field['charge'];
  ...
  public function element_render() {
    ...
    <?php if ( $this->is_admin_form ) : ?>
      <table><thead><tr><th align="left" colspan="2"><?php echo __( 'Override charge', 'wpf' ); ?></th><th align="left" colspan="2"><?php echo __( 'Override value', 'wpf' ); ?></th></thead><tr><td>
      ...
      <div class="override" style="display:<?php echo $this->is_overridden_value ? 'block' : 'none' ?>;">
        <input type='checkbox' class='wpf-widget-input wpf-field-value <?php echo $this->classes; ?>' name='<?php echo $this->name; ?>' <?php checked( $this->value ); ?> />
      </div>
      <div class="default" style="display:<?php echo $this->is_overridden_value ? 'none' : 'block' ?>;">
        <?php if ( $this->default_value ) : ?>
          <span title="By default is checked" class="dashicons dashicons-yes"></span>
        <?php else : ?>
          <span title="By default is unchecked" class="dashicons dashicons-no-alt"></span>
        <?php endif; ?>
      </div>
      </td></tr></table>
    <?php else : ?>      
      <input type='checkbox' name='<?php echo $this->name; ?>' data-id=<?php echo $this->id; ?> <?php if ( ! $this->is_admin_form ) : ?><?php else : ?><?php echo readonly( ! $this->is_overridden_value ); ?><?php endif; ?> <?php checked( 1 == $this->value || $this->value === 'on' ); ?> <?php echo wpf_attributes( $this->attributes ); ?> />
